...
def _normalize_stats(
    ...
    *,
    ...
    stats["ai_calls_saved_by_repeat_failure"] = _safe_int(data.get("ai_calls_saved_by_repeat_failure", 0))
    stats["estimated_tokens_saved"] = _safe_int(data.get("estimated_tokens_saved", 0))
    by_day = data.get("ai_calls_by_day")
    stats["ai_calls_by_day"] = dict(by_day) if isinstance(by_day, dict) else {}
...
def _maybe_monthly_reset(stats: Dict[str, Any], now_kst: datetime) -> Tuple[Dict[str, Any], bool]:
    ...
    if now_kst >= cutoff and stored_period != current_period:
        ...
        stats["ai_calls_saved_by_repeat_failure"] = 0
        stats["estimated_tokens_saved"] = 0
        # ai_calls_by_day 는 비용 리포트용으로 월 리셋 시에도 유지(꼬리만 별도 prune)
