...
# 구현은 legacy/kakao/ 에 있지만 자격증명(.env, kakao_code.json)은 btc_live_trading/ 에 유지한다.
_MODULE_DIR = Path(os.path.abspath(os.path.dirname(__file__)))
_PROJECT_ROOT = Path(os.path.realpath(str(_MODULE_DIR.parent.parent)))  # Coin/
_LIVE_DIR = Path(os.path.abspath(_PROJECT_ROOT / "btc_live_trading"))
KAKAO_CODE_JSON = Path(os.path.abspath(_LIVE_DIR / "kakao_code.json"))
...
def get_project_root() -> Path:
    """Coin 프로젝트 루트 (CWD 무관). legacy/kakao 기준 parent.parent."""
...
def _env_file_path() -> Path:
    return Path(os.path.abspath(_LIVE_DIR / ".env"))
